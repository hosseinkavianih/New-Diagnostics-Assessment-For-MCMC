...
setwd("/scratch/hk3sku/MCMC/HYMOD/outdreamnewwithupdate3")
...
# Calculating posterior matrix for each LH and random seed
       matrixposterior <- matrix(NA, Iterlengthtot, 7)
       ...
       for (p in 1:7) {
        ...
        for (c in 1:chainlength) {
         ...
         for (s in 1:Iterlength) {
          ...
          matrixposterior[s+Iterlength*(c-1), p] <- output$chain[[c]][s,p]
...
Posterior <- paste("/scratch/hk3sku/MCMC/HYMOD/PosteriorDREAM/Posterior", j, ".Rdata",sep="")
