...
def CDFs_sensitivity_100D(problem, metric, metric_name, figname):
    titles = []
    algNames = []
    ...
    #fig, plots = plt.subplots(nrows=1,ncols=len(problem.algorithms), constrained_layout=True, figsize=[15.5,5])
    fig = plt.figure()
    ...
    ax.legend(handles, labels, loc='upper right', fontsize=16, ncol=1, bbox_to_anchor=(1.8,1))
                
    norm = matplotlib.colors.Normalize(vmin, vmax)
    cbar_ax = fig.add_axes([0.87, 0.6, 0.02, 0.35])
    cbar = matplotlib.colorbar.ColorbarBase(cbar_ax, cmap=cmap, norm=norm)
    cbar.ax.tick_params(labelsize=14)
    cbar.set_ticks([vmin,vmax])
    cbar.set_ticklabels(['Min','Max'])
    cbar.ax.set_ylabel('Most Explanatory\nHyperparameter',fontsize=16)
